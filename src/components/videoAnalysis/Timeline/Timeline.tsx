    ...
    <div>
      ...
      >
        {options.map(option => (
          <MenuItem
            key={option.label}
            selected={option.value === moment.type}
            onClick={() => {
...
const Delete: React.FC<DeleteProps> = ({ delete: d }) => {
  return (
    <div>
      <IconButton aria-label='more' id='long-button' onClick={d}>
        <DeleteIcon />
      </IconButton>
    </div>
  );
  ...
  return (
    <div>
      <Stack direction={'row'}>
        ...
        >
          {showSlider ? <SaveIcon color='primary' /> : <AccessTimeIcon />}
        </IconButton>
...
const EventsTimeline: React.FC<TimelineProps> = props => {
  ...
  const updateMomentType = (moment: MeaningfulMoment) => (type: MomentType) => {
    setLocalEvents({
      ...localEvents,
      [moment.uuid]: {
        ...localEvents[moment.uuid],
  ...
  const toggleShowVideoButton = showVideo ? (
    ...
    <Button startIcon={<MovieIcon />} onClick={() => setShowVideo(true)}>
      Show video{' '}
    </Button>
  );
      ...
      <>
        <Stack>
          <Stack direction='row' spacing={0}>
            ...
            <Button startIcon={<AddIcon color='success' />} onClick={addMoment}>
              Add moment{' '}
            </Button>
            <CommonRowControls
                ...
                <TimelineItem key={e.startTime}>
                  ...
                  <TimelineContent sx={{ py: '12px', px: 2 }}>
                    ...
                    >
                      ...
                      <Options
                        moment={e}
                        updateMomentType={updateMomentType(e)}
                      />
                      <Delete delete={deleteMeaningfulMoment(e)} />
                      <TimePicker
                        time={e.startTime}
