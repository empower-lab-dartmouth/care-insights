...
const ProgramEventsTable: React.FC = () => {
  ...
  const updateMeaningfulMoments: (
    id: string
  ) => (moments: Record<string, MeaningfulMoment>) => void = id => moments =>
    setPageContext({
      ...pageContext,
      selectedCRProgramEvents: {
        ...pageContext.selectedCRProgramEvents,
        [id]: {
          ...(pageContext.selectedCRProgramEvents[id] as MusicProgramEvent),
          meaningfulMoments: moments,
        },
      },
    });
  const updateProgramEvent: (
    id: string
  ) => (programEvent: ProgramEvent) => void = id => programEvent => {
    setPageContext({
      ...pageContext,
      selectedCRProgramEvents: {
        ...pageContext.selectedCRProgramEvents,
