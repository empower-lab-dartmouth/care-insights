...
export const StreamGraph = ({ width, height, data }: StreamGraphProps) => {
  ...
  //
  const groups = [attentionColumnName, reactionColumnName, symptomColumnName, memoryRecallColumnName, emotionColumnName];
  ...
  const yScale = useMemo(() => {
    return d3.scaleLinear().domain([0, 1]).range([boundsHeight, 0]);
  }, [data, height]);
