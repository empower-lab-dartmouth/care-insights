...
export const parseTime = (dateString: string) => {
    const d = d3.timeParse("%Y-%m-%d")(dateString);
    if (d === null) {
        return (new Date());
    }
    return d;
...
export const StreamGraph = ({
  ...
  // X axis
  const dateDomain = d3.extent(filteredData.map((d) => parseTime(d.date))) as [Date, Date];
  const xScale = d3.scaleTime().domain(dateDomain).range([0, boundsWidth]);

// Heightened attention to program,
// Heightened physical engagement in program,
// Behavioral disturbance,
// Heightened memory recall,
// Heightened positive emotional response
  ...
  const allPath = series.map((serie, i) => {
    const path = areaBuilder(serie) as string;
    return (
      <path
        key={i}
      ...
      <svg width={width} height={height}>
        ...
        >
          ...
          <Labels
            ...
            xEnd={width}
            biggestValue={biggestValue as number}
          />
